...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['delete_bmi_id'])) {
        $bmi_id = $_POST['delete_bmi_id'];
        $delete_sql = "DELETE FROM bmi_records WHERE id = ? AND user_id = ?";
        $stmt = $conn->prepare($delete_sql);
        if ($stmt === false) {
            die('prepare() failed: ' . htmlspecialchars($conn->error));
        }
        $stmt->bind_param("ii", $bmi_id, $user_id);
        if (!$stmt->execute()) {
            die('execute() failed: ' . htmlspecialchars($stmt->error));
        }
        $stmt->close();
        $success_message = "BMI record deleted successfully.";
    }
}
?>
        ...
        <div class="bmi-history">
            ...
            if ($result->num_rows > 0) {
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    echo '<input type="hidden" name="delete_bmi_id" value="' . htmlspecialchars($row['id']) . '">';
